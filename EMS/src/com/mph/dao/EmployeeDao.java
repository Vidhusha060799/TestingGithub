...
public void type_scroll_sensitive_rs_update()
{
	con = MyDBConnection.getDBConnection();
	try {
		ps = con.prepareStatement("select eid,ename from mphemp where eid=2",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs = ps.executeQuery();
...
public void batchUpdate()
{
	...
	try {
		st = con.createStatement();
		st.addBatch("update mphemp set ename ='Ramyaa' where eid=4");
		st.addBatch("update mphemp set eid=2 where ename ='Ram'");
		st.addBatch("update mphemp set ename ='Anithaa' where eid=3");
		st.addBatch("update mphemp set ename ='Rahull' where eid=1");
